...
> **最新交接文档**：`HANDOFF-2026-09-23.md`（自包含，含 2026-09-23 全部变更与回归修复）。
> 本文件为 2026-09-17 旧版，保留更细的历史约定与验证矩阵，作为附录参考。

---

## 📌 最新变更 · InfiniSynapse 集成与系统优化（2026-09-23）

> 由 AI Agent 写入，供后续 Agent 与维护者阅读。

- **人读**：根目录 `INFINISYNAPSE_INTEGRATION.md`（批次 A–E 改动、接口、开关、验证、回退）。
- **机读**：根目录 `AGENT_CHANGES.json`（结构化变更清单：改动文件/新增文件/环境变量/端点/验证）。
- **Agent 记忆**：`.codebuddy/memory/2026-09-23.md`（+ `MEMORY.md` 指针）。
- **变更日志**：`CHANGELOG.md` 的 `[Unreleased]` 段。

一句话：AI 解读基于真实数据（facts 注入）；新增 SQL 计算引擎与 Skill 规范化层；
数据库索引/写入/连接优化；`init_db` 只跑一次、gzip/ETag、AI 超时对齐。
**对外零行为变更，`pytest` 74 passed，可一键回退。**
...
- **已发版（2026-09-18）**：`v1.1.0`=`7d3faab`（性能·加固·体验·SEO）、
  ...
  `v1.5.2`=`3b00c03`…`d07cf55`（SSE 双形态行为用例 + UI 质量收敛：指标卡左对齐+单位不折行+英文单位规范化+去 emoji+tab 键盘导航+代码债收敛）；
...
2. **数据标识符以中文规范键入库；对外本地化只在 `data/labels.csv` + `src/labels.py` 一处发生。**
   ...
   字典里仍保留的数据词表是**降级兜底**，必须与 `labels.csv` 逐字一致——
   这条已由 `tests/test_labels.py` 机器把守（改一侧不改另一侧即红），不再是口头约定。
...
1. ~~**零自动化测试、无 CI。**~~ ✅ **质量底线已建成三层**：`tests/` 下 6 个模块共 **125 项**检查
   ...
   **+ 冷启动自愈整条链路**、`test_embed` 4 项 embed 单遍合并回归、
   ...
   （node `vm` 跑真实 `public/app.js` 的 11 个前端行为场景 + 反向用例）、
   `conftest` 测试库隔离到临时目录）。
   `.github/workflows/ci.yml` 三个作业：
   `test`（重生成内嵌页 → **漂移即红** → `pytest` → 起本地服务跑 `scripts/check_i18n.py`）、
   `types`（**`basedpyright` 类型门禁**，此前只在 `pyproject.toml` 里声明、从未被执行）、
   `docs`（可选 `markdownlint-cli2`，不阻塞）。
...
3. **线上部署只做过接口探活。** 见 §5：`/api/stats`、`/api/overview` 已实测正常（2026-09-20），
   `vercel.json` 已由旧版 `routes` 迁到 `rewrites`（语义等价，消除 `Due to builds existing in
   your configuration file...` 警告），但**页面级路由（`/` `/app` `/docs` `/privacy`
   `/robots.txt` `/sitemap.xml` `/og-image.png`）未逐一复核**，下次上线时顺手确认。
