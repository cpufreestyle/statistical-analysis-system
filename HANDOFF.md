...
| 类型门禁（`basedpyright`） | ✅ 已接入并全绿 | 接入时暴露 **10 处**类型缺陷已全部修掉，现在 `0 errors, 0 warnings, 0 notes`；反面：临时塞一个 `return "str"` 到 `src/` → 立刻报 1 error（探针已删除）。CI `types` 作业锁 `basedpyright==1.39.9` |
